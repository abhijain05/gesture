...
export default function Reports() {
  ...
  const kpis = [
    { label: "Total",       value: REPORTS.length,                                             color: "#0070f2" },
    ...
    { label: "Downloads",   value: REPORTS.reduce((s, r) => s + r.downloads, 0).toLocaleString(), color: "#6c3483" },
  ];

  return (
    <div className="page-pad" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        <h1 style={{ margin: 0, fontSize: isMobile ? 20 : 24, fontWeight: 700, color: "#1d2d3e" }}>Reports</h1>
        <p style={{ margin: "4px 0 0", color: "#6a6d70", fontSize: 13 }}>
          {filtered.length} of {REPORTS.length} reports · Updated just now
        </p>
      </div>

      {/* KPI strip — 4 cols desktop, 2×2 mobile */}
      <div className="grid-kpi-4">
        {kpis.map((kpi) => (
          <div key={kpi.label} className="sap-card" style={{ padding: "12px 14px" }}>
            <div style={{ fontSize: 11, color: "#6a6d70", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>
              {kpi.label}
            </div>
      ...
      {/* Search + filter bar */}
      <div className="sap-card" style={{ padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ position: "relative" }}>
          ...
          <input
            ...
            style={{
              ...
              border: "1px solid #d0d0d0", borderRadius: 7, fontSize: 13,
              outline: "none", boxSizing: "border-box",
              background: "#fafafa", color: "#1d2d3e",
            }}
        ...
        <div style={{ display: "flex", gap: 6, overflowX: "auto", WebkitOverflowScrolling: "touch" as any, paddingBottom: 2 }}>
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
              data-gesture-dwell
              ...
              style={{
                ...
                border: "1.5px solid", borderColor: category === cat ? "#0070f2" : "#d0d0d0",
      ...
      <div className="sap-card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        {!isMobile && (
          <div style={{
            ...
            padding: "9px 16px", background: "#f5f6f7", borderBottom: "1px solid #e8e8e8",
            fontSize: 11, fontWeight: 700, color: "#6a6d70", textTransform: "uppercase", letterSpacing: "0.06em",
          }}>
            <span>Title</span>
            <span>Category</span>
            <span>Date</span>
        ...
        {filtered.length === 0 ? (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                    <div style={{ flex: 1, marginRight: 8 }}>
                      <div style={{ fontWeight: 600, fontSize: 13, color: "#0070f2", lineHeight: 1.4 }}>{report.title}</div>
                      <div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>by {report.author}</div>
                    </div>
                    <span style={{
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 10, fontSize: 11, fontWeight: 600, background: catStyle.bg, color: catStyle.color }}>
                      {report.category}
                    </span>
          ...
          filtered.map((report, i) => {
            ...
            return (
              ...
              >
                <div>
                  <div style={{ fontWeight: 600, fontSize: 13, color: "#0070f2", marginBottom: 2 }}>{report.title}</div>
                  <div style={{ fontSize: 11, color: "#888" }}>by {report.author}</div>
                </div>
                <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 12, fontSize: 11, fontWeight: 700, background: catStyle.bg, color: catStyle.color }}>
                  {report.category}
                </span>
                ...
                <div style={{ textAlign: "center" }}>
                  <span style={{ display: "inline-block", padding: "2px 9px", borderRadius: 10, fontSize: 11, fontWeight: 600, background: report.status === "Published" ? "#e8f5e9" : "#fff3e0", color: report.status === "Published" ? "#2e7d32" : "#e65100" }}>
                    {report.status}
                  </span>
                </div>
